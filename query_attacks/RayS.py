...
class RayS(object):
    ...
    def attack_hard_label(self, x, y, target=None, query_limit=10000, seed=None):
        ...
        for i in range(query_limit):
            ...
            if i % 10 == 0:
                ...
                succ = (dist <= self.epsilon)
                acc = 1 - succ.float().mean().item()
                progress_bar(torch.min(self.queries.float()), query_limit, 'acc: %.4f' % (acc))
    ...
    def search_succ(self, x, y, target, mask):
        ...
        if target:
            ...
            return self.model(x[mask]).argmax(-1) == target[mask]
        else:
            # return self.model.predict_label(x[mask]) != y[mask]
            return self.model(x[mask]).argmax(-1) != y[mask]
